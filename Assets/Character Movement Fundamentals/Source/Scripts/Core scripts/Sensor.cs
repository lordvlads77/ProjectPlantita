...
namespace CMF
{
	...
	public class Sensor {
		...
		//Constructor;
		public Sensor (Transform _transform, Collider2D _collider)
		{
			tr = _transform;
		...
		private void CastRayArray(Vector3 _origin, Vector3 _direction)
		{
			...
			for(int i = 0; i < raycastArrayStartPositions.Length; i++)
			{
				...
				_hit = Physics2D.Raycast(_rayStartPosition, rayDirection, castLength, layermask);
				if(_hit.collider)
				{
					if(isInDebugMode)
		...
		private void CastRay(Vector3 _origin, Vector3 _direction)
		{
			...
			RaycastHit2D _hit = Physics2D.Raycast(_origin, _direction, castLength, layermask);
		...
		private void CastSphere(Vector2 _origin, Vector2 _direction)
		{
			throw new NotImplementedException("Falta actualizar esto a 2D");
			/*RaycastHit2D _hit = Physics.SphereCast(_origin, sphereCastRadius, _direction, castLength - sphereCastRadius, layermask);

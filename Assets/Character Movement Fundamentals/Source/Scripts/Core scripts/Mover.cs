...
namespace CMF
{
	...
	public class Mover : MonoBehaviour {
		...
		void Setup()
		{
			...
			if(col == null)
			{
				col= tr.gameObject.AddComponent<Collider2D>();
			}
			...
			if(rig == null)
			{
				rig= tr.gameObject.AddComponent<Rigidbody2D>();
			}
		...
		public void RecalculateColliderDimensions()
		{
			...
			if(capsuleCollider)
			{
				...
				capsuleCollider.offset = capsuleCollider.offset + new Vector2(capsuleCollider.offset.x, stepHeightRatio * capsuleCollider.size.y/2f);
				capsuleCollider.size = new Vector2(capsuleCollider.size.x,capsuleCollider.size.y * (1f - stepHeightRatio));
				if(capsuleCollider.size.y/2f < capsuleCollider.offset.x)
					capsuleCollider.offset = new Vector2(capsuleCollider.size.y/2f, capsuleCollider.offset.y);
		...
		void RecalculateSensorLayerMask()
		{
            ...
            for (int i = 0; i < 32; i++)
            {
                if (!Physics2D.GetIgnoreLayerCollision(_objectLayer, i)) 
